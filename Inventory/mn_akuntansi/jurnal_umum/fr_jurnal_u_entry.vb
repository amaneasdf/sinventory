Public Class fr_jurnal_u_entry
    ...
    Private Sub SetUpForm(IdJurnal As String, FormSet As InputState, AllowInput As Boolean)
        Const _tempTitle As String = "Detail Jurnal Umum : JU.ENT.xxx"
        formstate = FormSet
        With cb_ppn
                    ...
                    If formstate = InputState.Insert Then
                        If String.IsNullOrWhiteSpace(in_kode.Text) Then
                            ...
                            q = "SELECT IFNULL(MAX(SUBSTRING_INDEX(line_kode, '.', -1)),0) FROM data_jurnal_line " _
                                & "WHERE line_kode REGEXP 'JU.ENT.[[:digit:]]{3,93}$'"
                            i = CInt(x.ExecScalar(q))
                            format = IIf(i + 1 > 999, "D" & (i + 1).ToString.Length, "D3")
                            in_kode.Text = String.Format("JU.ENT.{0}", (i + 1).ToString(format))

                        Else
                            'CHECK INPUTED CODE
                            Dim _ckCode As Integer = 0
                            q = "SELECT COUNT(line_kode) FROM data_jurnal_line WHERE line_kode='{0}' AND line_kat='UMUM' AND line_type='ENTRY1'"
                        ...
                        _col.AddRange({"line_kode='" & in_kode.Text & "'",
                                       ...
                                       "line_ref_type='TEXT'", "line_type='ENTRY1'", "line_kat='UMUM'"
                                      })
    ...
    Private Sub deletEntryJurnal()
        ...
        Using x = MainConnection
            x.Open() : If x.ConnectionState = ConnectionState.Open Then
                ...
                If _ck Then
                    MessageBox.Show("Entry jurnal umum telah dihapus.", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Information)
                    DoRefreshTab_v2({pgjurnalsesuai})
                    Me.Close()
                Else
                    MessageBox.Show("Entry jurnal umum gagal dihapus.", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
                End If
            Else
                MessageBox.Show("Tidak dapat terhubung ke database.", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
            End If
    ...
    Private Sub mn_delete_Click(sender As Object, e As EventArgs) Handles mn_delete.Click
        Me.Cursor = Cursors.WaitCursor
        If MessageBox.Show("Hapus data entry jurnal umum?", Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            deletEntryJurnal()
        End If
        Me.Cursor = Cursors.Default
    End Sub
