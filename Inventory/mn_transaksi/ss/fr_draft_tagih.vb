Public Class fr_draft_tagih
    ...
    'LOAD FAKTUR
    Private Sub loadFaktur(sales As String)
        If MainConnection.Connection Is Nothing Then
            Throw New NullReferenceException("Main DB Connection is empty")
        End If

        Dim q, qwhr, qtgl, qorder As String
        Dim whr As New List(Of String)
        Dim whr_tgl As New List(Of String)
        q = "SELECT piutang_faktur, piutang_tgl, getPiutangSisa(piutang_faktur) as piutang_sisa, " _
            & "piutang_jt, GetMasterNama('custo', piutang_custo) piutang_custo_n, GetMasterNama('custopsr', piutang_custo) customer_pasar, " _
            & "GetMasterNama('custokec', piutang_custo) customer_kecamatan, GetMasterNama('custokab', piutang_custo) customer_kabupaten, salesman_nama " _
            & "FROM data_piutang_awal " _
            & "LEFT JOIN data_salesman_master ON salesman_kode=piutang_sales " _
            & "WHERE piutang_status<9 AND piutang_sales='{0}'"
        q = String.Format(q, sales)

        If Not String.IsNullOrWhiteSpace(in_cari_faktur.Text) Then
            whr.Add("piutang_faktur REGEXP '" & Trim(in_cari_faktur.Text).Replace(" ", ".+") & "'")
        End If
        If Not String.IsNullOrWhiteSpace(in_kabupaten.Text) Then
            whr.Add("customer_kabupaten REGEXP '" & Trim(in_kabupaten.Text).Replace(" ", ".+") & "'")
        End If
        If Not String.IsNullOrWhiteSpace(in_kecamatan.Text) Then
            whr.Add("customer_kecamatan REGEXP '" & Trim(in_kecamatan.Text).Replace(" ", ".+") & "'")
        End If
        If Not String.IsNullOrWhiteSpace(in_pasar.Text) Then
            whr.Add("customer_pasar REGEXP '" & Trim(in_pasar.Text).Replace(" ", ".+") & "'")
        End If
        If ck_hidepaid.Checked = True Then
            whr.Add("getPiutangSisa(piutang_faktur) <> 0")
        End If
        qwhr = IIf(whr.Count > 0, " AND " & String.Join(" AND ", whr), "")

        If ck_tgl2.Checked = True Then
            whr_tgl.Add(String.Format("piutang_jt BETWEEN '{0:yyyy-MM-dd}' AND '{1:yyyy-MM-dd}'", date_faktur_awal.Value, date_faktur_akhir.Value))
        End If
        If ck_tgl1.Checked = True Then
            whr_tgl.Add(String.Format("piutang_jt < '{0:yyyy-MM-dd}'", date_faktur_awal.Value))
        End If
        qtgl = IIf(whr_tgl.Count > 0, String.Format(" AND ({0})", String.Join(" OR ", whr_tgl)), "")

        qorder = " ORDER BY piutang_faktur LIMIT 500"

        q += qwhr & qtgl & qorder

        Using x = MainConnection
            x.Open() : If x.ConnectionState = ConnectionState.Open Then
                dgv_listfaktur.DataSource = x.GetDataTable(q)
            Else
