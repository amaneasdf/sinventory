...
Module functional
    ...
    'IO FILE/DIR
    'CREATE TEXT FILE
    Public Sub CreateTextFile(FileDirectory As String, FileName As String, Contents As List(Of String), Optional AppendFile As Boolean = True)
    ...
    'WRITE VALUE TO INI CONFIG FILE
    ''' <summary>
    ...
    'EXPORT CSV
    Public Function ExportCSV(Filename As String,
                               FieldName As String(),
                               ExportData As DataTable,
        ...
        contain.Add(String.Join(FieldDelimiter, FieldName))
        For Each _row As DataRow In ExportData.Rows
            contain.Add(_row.ToCsv(False, FieldDelimiter))
        Next

        System.IO.File.WriteAllText(Filename, String.Join(RowDelimiter, contain))

        'CHECKING IF FILE SUCCESSFULY GENERATED
        Dim _respBool = System.IO.File.Exists(Filename)
        If _respBool Then
            Return True
        Else
            Throw New Exception("Cannot find generated file.", New Exception(Filename & " doesnt exist."))
        End If
    End Function
...
NextTab:
    ...
    'EXPORT EXCEL
    Public Function ExportExcel(ColHeader As List(Of String),
                                DataTbl As List(Of DataTable), DataTitle As List(Of String),
    ...
    Public Sub errLog(errList As List(Of String))
        ...
        CreateTextFile(AppDir_Document & "log\", file, errList, True)
        logErrToDb(errList)
    End Sub
End Module
